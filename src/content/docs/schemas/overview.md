...
A [network](/concepts/vocabulary#network) schema is the runtime contract for a DPG network. It tells the API and UI what the network is, which roles exist, what each role can publish, where those records are served, and which interactions are allowed.
...
| `item_schemas` | JSON Schemas for records a [domain](/concepts/vocabulary#domain) can publish |
| `instances` | Registered API [instances](/concepts/vocabulary#instance) that serve a domain |
