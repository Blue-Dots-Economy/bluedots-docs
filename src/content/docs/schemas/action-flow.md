...
```mermaid
stateDiagram-v2
    [*] --> Validating: Client POSTs action request
    Validating --> Failed: Request schema violation
    Validating --> Processing: Schema valid
    Processing --> EventEmitted: Handler succeeds
    Processing --> Failed: Handler error or permission denied
    EventEmitted --> [*]: Event stored, 200 returned
    Failed --> [*]: Error response returned
```
