---
title: Use Cases
description: Concrete scenarios where Blue Dots turns local proximity into discoverable, actionable matches.
sidebar:
  order: 1
next:
  link: /explore/pilots/
  label: "Path 4 of 7: Pilots"
---

import JourneyQuote from '../../../components/JourneyQuote.astro';

Blue Dots applies wherever seekers and providers are close in distance but invisible to each other. Livelihoods is the first and most developed use case; the same rails extend across sectors.

:::note
The personas below — the jobseeking youth, the woman seeking nearby work, the citizen chasing a scheme, the person with a disability looking for support — each map to a colour-coded member of the "Dots family." For the full taxonomy, see [Beyond Livelihoods: The Dots Family](/explore/beyond-livelihoods/).
:::

## Livelihoods for youth

The flagship use case. An ITI graduate looking for trade-specific work and a small business 800 metres away that needs exactly that skill currently never connect.

<JourneyQuote
  before="I guess I will have to move to a city to find a job."
  after="I didn't realize there are 100s of relevant jobs in my vicinity!"
  persona="Youth seeking livelihoods"
/>

With Blue Dots:

- The graduate publishes a Blue Dot (often by voice).
- The employer's need is captured as a complementary signal.
- The network surfaces the match within a day, two bus stops apart — keeping income and spending inside the district.

**Pilot evidence:** 10,000+ jobs surfaced from under 10% of local SMBs; discovery time cut from over two weeks to under two days; conversion around 50% in targeted drives, against around 5% conventional. See [Pilots](/explore/pilots/). <span class="bd-new" title="Added/updated from the adopter-guide PDF">new</span>

## Women's workforce participation

Local, proximity-aware discovery lowers the barriers that keep women's labour-force participation under 35% — surfacing nearby, suitable work without long-distance migration.

<span class="bd-new" title="Added/updated from the adopter-guide PDF">new</span>

Field data sharpens what "suitable" means. Around **75% of women will not proceed without clarity on three things**: safety, closeness to home, and workplace amenities or shift timings. Women also search far closer to home than men — roughly two bus stops, against a district-wide search. Making those assurances visible *before* someone applies is as important as surfacing the role. See [What We're Learning](/explore/learnings/#7-women-are-asking-for-specific-assurances-before-taking-a-job).

<JourneyQuote
  before="I don't want to travel more than 20 minutes for work."
  after="Spoke for 2 mins and I can now see remote work and roles just two bus stops away."
  persona="Women re-entering the workforce"
/>

## Government scheme uptake

Scheme budget utilisation sits below 70% largely because eligible citizens never discover relevant schemes. Modelling schemes as providers and citizens as seekers makes entitlements discoverable at the local level.

## Local services

Service uptake runs at under 50% of potential. Blue Dots connects citizens with nearby services they didn't know existed — the same discovery pattern, a different domain. This is especially powerful for persons with disabilities, who often don't know where to turn for the support that would let them start working.

<JourneyQuote
  before="I don't know who to talk to for the support I need to start working."
  after="I just highlighted my needs and I can already see options for both work and services!"
  persona="Persons with disabilities"
/>

## The common pattern

Every use case is the same shape:

{/* Editable source: src/assets/diagrams/use-cases-common-pattern.excalidraw — open at https://excalidraw.com to adjust, re-export PNG here. */}

![A seeker signal discovers a provider signal; an action between them produces an event — the outcome](../../../assets/diagrams/use-cases-common-pattern.png)

Adding a new use case means defining the **domain** and **item schemas** for it — not building a new system. See [Beyond Livelihoods](/explore/beyond-livelihoods/).
