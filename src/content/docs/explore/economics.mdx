...
description: How better local discovery adds ~₹87,500 crore of annual economic activity — the full derivation from Ghaziabad to India's top 100 districts.
sidebar:
  order: 4
prev:
...
next:
  link: /explore/beyond-livelihoods/
  label: "Path 7 of 7: The Dots Family"
---
...
<StatGrid>
  <StatCard value="₹87,500 cr" label="Estimated annual economic activity unlock, India's top 100 districts" />
  <StatCard value="~70,000" label="New workers added per district (conservative 5% of latent workforce, Ghaziabad)" color="green" />
  <StatCard value="1.5×" label="Local spending multiplier (Bartik 2019 — conservative restatement)" color="blue" />
  <StatCard value="50×" label="Scale factor from Ghaziabad to India's top 100 districts by population" color="purple" />
</StatGrid>

## Core logic

Better local discovery addresses the [paradox of proximity](/overview/paradox-of-proximity/) by adding workers who were always present but invisible. Once discoverable, they **earn locally, spend locally**, and that spending creates further activity through a **local multiplier**.
...
A conservative **5%** of each group finds work through better local discovery:
...
Lower-income and first-time earners retain a high share of income locally. Per-worker annual spend distribution (illustrative, on a ~₹1.5 lakh income):
...
Each ₹1 of local spending generates ~₹1.5 of local economic activity (retail, housing, transport, education, services).
...
Ghaziabad's population is ~4.7M. India's top 100 districts (excluding metros) total ~250M population, giving a conservative scaling factor of **50×**.
...
:::caution[Caveats]
Districts with weaker MSME density or local linkages may generate less; stronger cluster districts more — **Ghaziabad is a reasonable mid-point**. The estimate also excludes compounding from repeat local-spending cycles.
:::

## What this calculation excludes

The headline figure is deliberately conservative. It leaves out three additive channels:

1. **Welfare & scheme utilisation** — faster discovery lifts utilisation for a *different* population, so the effect is additive, not overlapping.
2. **Services activation** — visible local providers (teachers, repair, care, tourism, agri-services) raise service-consumption GDP.
3. **MSME productivity & revenue** — faster hiring and customer discovery improves output.
...
See also: [The Paradox of Proximity](/overview/paradox-of-proximity/) · [Pilots](/explore/pilots/)
